...
class TransformerEnhancedVedicAnalyzer:
    ...
    def _analyze_with_transformer(self, text: str, text_name: str) -> Dict:
            ...
            try:
                ...
                with torch.no_grad():
                    outputs = self.transformer_analyzer(
                        ...
                        return_attention=False,  # For faster inference
                    )
        ...
        return {"features": normalized_features, "confidences": averaged_confidences}

    def _extract_features_from_outputs(
        self, outputs: Dict, sentence: str, confidence: float
    ) -> Dict:
        ...
        for word in words:
            ...
            if word in ["ātman", "brahman", "mokṣa"] and confidence > 0.6:
                if any(
                    phil_word in sentence for phil_word in ["jñāna", "vidyā", "satya"]
                ):
    ...
    def _combine_analyses(
        ...
        for feature in all_features:
            ...
            if self.ensemble_method == "weighted_average":
                ...
                ensemble_freq = (
                    min(regex_freq, transformer_freq)
                    if transformer_freq > 0
                    else regex_freq
                )
    ...
    def generate_enhanced_report(
        self, output_path: str = "enhanced_diachronic_report.txt"
    ) -> str:
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            f.write("-" * 50 + "\n")
            f.write(
                "• Dual analysis: Transformer neural network + Regex pattern matching\n"
            )
            ...
            for category, trends in trend_analysis.items():
                ...
                for feature, trend_data in trends.items():
                    ...
                    f.write(
                        f"(R²={r_squared:.3f}, p={p_value:.3f}{significance}, conf={confidence:.2f})\n"
                    )
            ...
            if self.use_transformer:
                        ...
                        if common_features:
                            ...
                            for feature in common_features:
                                if regex_res[feature] > 0 or trans_res[feature] > 0:
                                    rel_diff = abs(
                                        regex_res[feature] - trans_res[feature]
                                    ) / max(regex_res[feature], trans_res[feature], 0.1)
                                    correlations.append(
                                        1 - rel_diff
                ...
                if agreements:
                    ...
                    f.write(
                        f"Average Regex-Transformer Agreement: {avg_agreement:.3f}\n"
                    )
    ...
    def export_enhanced_results(
        self, output_path: str = "enhanced_vedic_analysis.csv"
    ) -> str:
