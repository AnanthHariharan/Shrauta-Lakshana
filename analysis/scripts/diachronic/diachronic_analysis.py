...
class EnhancedVedicAnalyzer(VedicDiachronicAnalyzer):
    ...
    def analyze_feature_categories(self, text_name):
        ...
        categories = {
            ...
            "morphological_archaic": [
                ...
                "n_infix_presents",
                "reduplicated_presents",
                "locative_dual",
                "locative_plural",
            ],
            "morphological_innovative": [
                "perfect_periphrastic",
                "instrumental_classical_ena",
                "genitive_plural_thematic",
                "precative",
                "benedictive",
            ],
            "syntactic_archaic": [
            ...
            "syntactic_innovative": [
                "present_participle_ant",
                "correlatives_ya_ta",
                "conditional_yadi_tarhi",
                "long_compounds",
                "gerund_tvaa",
                "infinitive_tum",
        ...
        for category, features in categories.items():
            scores = [
                results.get(feature, 0) for feature in features if feature in results
            ]
    ...
    def calculate_archaism_innovation_indices(self, text_name):
        ...
        archaic_score = sum(results.get(f, 0) for f in archaic_features) / len(
            archaic_features
        )
        innovative_score = sum(results.get(f, 0) for f in innovative_features) / len(
            innovative_features
        )
        ...
        return {
            ...
            "conservation_ratio": archaic_score / innovative_score
            if innovative_score > 0
            else float("inf"),
        }
    ...
    def calculate_advanced_composite_indices(self, text_name):
        ...
        return {
            ...
            "overall_innovation_score": (
                morph_innovative_density + syntactic_complexity + textual_sophistication
            )
            / 3,
        }
        ...
        def plot_diachronic_trends(analyzer):
            ...
            for idx, feature in enumerate(features_to_plot):
                if idx < len(axes):
                    ...
                    ax.scatter(
                        range(len(text_order)), values, c=colors, s=100, alpha=0.6
                    )
...
def plot_diachronic_trends(analyzer):
    ...
    for idx, (category, features) in enumerate(feature_categories.items()):
        if idx < len(axes):
            ...
            ax.axvspan(
                4.5, 8.5, alpha=0.1, color="blue", label="Brahmana Period"
            )  # 5-8
            ax.axvspan(
                8.5, 14.5, alpha=0.1, color="green", label="Upanishad Period"
            )  # 9-14
            ax.axvspan(
                14.5, 17.5, alpha=0.1, color="yellow", label="Classical Period"
            )  # 15-17
    ...
    plt.tight_layout()
    plt.savefig(
        "../../../output/vedic_comprehensive_diachronic_analysis.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.show()
...
def plot_category_trends(analyzer):
    ...
    if category_data:
        ...
        plt.tight_layout()
        plt.savefig(
            "../../../output/vedic_category_trends.png", dpi=300, bbox_inches="tight"
        )
        plt.show()
...
def statistical_analysis(analyzer):
    ...
    for feature in features:
        ...
        samhita_avg = np.mean(
            [analyzer.results[t][feature] for t in periods["Samhita"]]
        )
        upanishad_avg = np.mean(
            [analyzer.results[t][feature] for t in periods["Upanishad"]]
        )
...
def export_results(analyzer, filename="../../../output/vedic_analysis.csv"):
    """Export results for further analysis"""
    df = pd.DataFrame(analyzer.results).T

    # Updated: Now 20 texts (6 Samhitas + 5 Brahmanas + 6 Upanishads + 3 Classical)
    periods = (
        ["Early Vedic"] * 6
        + ["Late Vedic"] * 5
        + ["Latest Vedic"] * 6
    ...
    # Samhitas: RV, SV, YV-T, YV-M, AV-P, AV-S | Brahmanas: Kaus, Panc, Tait, Sata, Gopa
    dates = (
        [1500, 1200, 1000, 1000, 800, 600]
        + [900, 850, 850, 800, 750]
        + [700, 650, 600, 550, 480, 450]
        + [200, 300, 800]
    )
    ...
    df["archaism_index"] = (
        df["retroflex_l"] + df["subjunctive_ati"] + df["particle_sma"]
    ) / 3
    ...
    features_df.to_csv("../../../output/features_over_time.csv")
...
def analyze_with_sanskrit_tools(text_file):
    try:
        ...
        for word in words[:100]:
            try:
                word_dev = sanscript.transliterate(
                    word, sanscript.IAST, sanscript.DEVANAGARI
                )
...
def create_comprehensive_report(analyzer):
    ...
    with open("vedic_comprehensive_diachronic_report.txt", "w", encoding="utf-8") as f:
        ...
        f.write("-" * 50 + "\n")
        f.write(
            "• Samhitas (c. 1500-1000 BCE): Rigveda, Yajurveda, Samaveda, Atharvaveda\n"
        )
        f.write(
            "• Brahmanas (c. 900-750 BCE): Kausitaki, Pancavimsa, Satapatha, Gopatha\n"
        )
        f.write(
            "• Upanishads (c. 700-550 BCE): Brhadaranyaka, Chandogya, Aitareya, Mandukya\n\n"
        )
        ...
        if hasattr(analyzer, "analyze_diachronic_trends"):
            ...
            increasing = [
                f
                for f, data in trend_analysis.items()
            ...
            stable = [
                f for f, data in trend_analysis.items() if data["direction"] == "stable"
            ]

            f.write(f"INCREASING FEATURES ({len(increasing)}):\n")
            for feature in sorted(
                increasing,
                key=lambda x: trend_analysis[x]["change_percent"],
                reverse=True,
            )[:10]:
                data = trend_analysis[feature]
                f.write(
                    f"  • {feature.replace('_', ' ')}: {data['change_percent']:+.1f}% change\n"
                )

            f.write(f"\nDECREASING FEATURES ({len(decreasing)}):\n")
            for feature in sorted(
                decreasing,
                key=lambda x: abs(trend_analysis[x]["change_percent"]),
                reverse=True,
            )[:10]:
                data = trend_analysis[feature]
                f.write(
        ...
        for category, features in categories.items():
            ...
            for feature in features:
                if feature in analyzer.features:
                    values = [
                        analyzer.results[text].get(feature, 0) for text in text_order
                    ]
                    ...
                    if early_avg > 0:
                        ...
                        f.write(
                            f"  {trend} {feature.replace('_', ' ')}: {early_avg:.2f} → {late_avg:.2f} ({change:+.1f}%)\n"
                        )
        ...
        for period, texts in periods.items():
            if hasattr(analyzer, "calculate_archaism_innovation_indices"):
                ...
                avg_ratio = np.mean(
                    [
                        idx["conservation_ratio"]
                        for idx in indices
                        if idx["conservation_ratio"] != float("inf")
                    ]
                )
        ...
        for period, texts in periods.items():
            if hasattr(analyzer, "calculate_advanced_composite_indices"):
                composite_indices = []
                for text in texts:
                    composite_indices.append(
                        analyzer.calculate_advanced_composite_indices(text)
                    )

                avg_morph_innovation = np.mean(
                    [
                        idx["morphological_innovation_density"]
                        for idx in composite_indices
                    ]
                )
                avg_syntactic_complexity = np.mean(
                    [idx["syntactic_complexity"] for idx in composite_indices]
                )
                avg_phonological_archaism = np.mean(
                    [idx["phonological_archaism"] for idx in composite_indices]
                )
                avg_textual_sophistication = np.mean(
                    [idx["textual_sophistication"] for idx in composite_indices]
                )
                avg_substrate_influence = np.mean(
                    [idx["substrate_influence"] for idx in composite_indices]
                )
                avg_overall_innovation = np.mean(
                    [idx["overall_innovation_score"] for idx in composite_indices]
                )

                f.write(f"{period}:\n")
                f.write(
                    f"  Morphological Innovation Density: {avg_morph_innovation:.2f}\n"
                )
        ...
        f.write(
            "• Comprehensive coverage of phonological, morphological, syntactic, and lexical domains\n"
        )
        f.write(
            "• Advanced phonological features: pluti vowels, voiced aspirate retention, cluster simplification\n"
        )
        f.write(
            "• Morphological archaisms: N-infix presents, reduplicated presents, archaic case forms\n"
        )
        f.write(
            "• Syntactic evolution: absolute constructions, word order patterns, subordination markers\n"
        )
        f.write(
            "• Lexical stratification: substrate influence, philosophical terminology co-occurrence\n"
        )
        f.write(
            "• Textual sophistication: prose vs verse markers, reported speech, connectives\n"
        )
        f.write(
            "• Composite indices: innovation density, syntactic complexity, textual sophistication\n\n"
        )
