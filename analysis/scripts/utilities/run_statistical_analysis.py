...
# Add parent directory to path to import from diachronic module
script_dir = Path(__file__).parent.absolute()
diachronic_dir = script_dir.parent / "diachronic"
statistical_dir = script_dir.parent / "statistical"
sys.path.insert(0, str(diachronic_dir))
sys.path.insert(0, str(statistical_dir))

from diachronic_analysis import EnhancedVedicAnalyzer
from statistical_modeling import (
    VedicTextMetadata,
    VedicStatisticalModeler,
    VedicStatisticalVisualizer,
)
import pandas as pd
import numpy as np
...
def generate_statistical_report(
    ...
    with open("vedic_statistical_analysis_report.txt", "w", encoding="utf-8") as f:
        ...
        f.write("-" * 50 + "\n")
        f.write(
            "This report presents statistical modeling results for Vedic Sanskrit\n"
        )
        f.write(
            "diachronic analysis, using text characteristics (prose/poetry, period,\n"
        )
        ...
        if model_results:
            ...
            for target, (model, r2) in sorted(
                best_models.items(), key=lambda x: x[1][1], reverse=True
            ):
                ...
                for model_name, validation in models.items():
                    if validation["recommended"]:
                        recommended_models.append(
                            (target, model_name, validation["interpretation"])
                        )
        ...
        if pca_results:
            ...
            f.write(
                f"First 5 components explain {sum(explained_var[:5]):.1%} of variance:\n"
            )
        ...
        f.write("-" * 50 + "\n")
        f.write(
            "• Linear regression, Ridge, Lasso, Random Forest, Gradient Boosting models\n"
        )
        f.write("• Cross-validation for model selection and validation\n")
        f.write(
            "• Feature importance analysis via coefficients and tree-based methods\n"
        )
        ...
        f.write(
            "Statistical modeling demonstrates quantitative relationships between\n"
        )
